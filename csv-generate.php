...
while ($data = mysqli_fetch_assoc($all_results)) {
    ...
    array_push($csv_result, $new_row); //add name and marks

}

$exam_name_slug = strtolower(str_replace(' ','-',$exam_name));  
$save_file_name = $exam_name_slug.'-'.time().'.csv';  //save file name

array_to_csv_download($csv_result, $save_file_name);   //method bellow
